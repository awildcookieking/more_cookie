...
public class SaphirestaffInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	@SubscribeEvent
	@OnlyIn(Dist.CLIENT)
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntityItem().getItem().getItem() == BossrushModItems.SAPHIRESTAFF.get()) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(1) {
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				return stack.getItem() != BossrushModItems.SAPHIRESTAFF.get();
			}
