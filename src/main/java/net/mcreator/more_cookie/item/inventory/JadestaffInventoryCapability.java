...
public class JadestaffInventoryCapability implements ICapabilitySerializable<CompoundTag> {
	@SubscribeEvent
	@OnlyIn(Dist.CLIENT)
	public static void onItemDropped(ItemTossEvent event) {
		if (event.getEntityItem().getItem().getItem() == BossrushModItems.JADESTAFF.get()) {
	...
	private ItemStackHandler createItemHandler() {
		return new ItemStackHandler(1) {
			...
			public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
				return stack.getItem() != BossrushModItems.JADESTAFF.get();
			}
